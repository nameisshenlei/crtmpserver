...
bool BaseRTMPAppProtocolHandler::ProcessInvokePlay(BaseRTMPProtocol *pFrom,
	...
	//lshen修改
	size_t offset = 0;
	while (offset = streamName.find_first_of('?', offset), offset != string::npos)
		streamName[offset] = PATH_SEPARATOR;
	M_INVOKE_PARAM(request, 1) = streamName;
	trim(streamName);
	if (streamName == "") {
		FATAL("Invalid request:\n%s", STR(request.ToString()));
...
Variant BaseRTMPAppProtocolHandler::GetMetaData(string streamName,
	...
	if (parts.size() == 1) {
		...
		//lshen添加
		string mediatype = result[META_MEDIA_TYPE];
		if (mediatype.compare("ev") == 0)
			result[META_MEDIA_TYPE] = MEDIA_TYPE_LIVE_OR_FLV;
	}
	...
	{
		//lshen修改
		if (parts.size() > 1 && parts[0].compare("ev") == 0)
			searchFor = parts[1];
		else
